...
async function main() {
  ...
  fs.writeFileSync(
    deploymentFile,
    JSON.stringify({ ZombieOwnership: contractAddress }, null, 2)
  );
  console.log("Saved address to:", deploymentFile);
  console.log(`Use: $env:CONTRACT_ADDRESS="${contractAddress}" (optional — scripts read the file automatically)\n`);
